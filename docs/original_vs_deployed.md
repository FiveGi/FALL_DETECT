...
**It cannot be fed every frame.** Timed in the same process on the same 1080p frames,
the original needs **70.3 ms per frame against the deployed pipeline’s 54.4 ms** —
MediaPipe on the CPU is **1.3x slower** than YOLO26s-pose on the GPU, before decoding
or anything else in the loop. Those two figures are from a host Python and are not
deployment rates: the same deployed code measured inside its own container takes
**26.3 ms per frame**, and the live camera loop with decoding, clip buffering and the
database sustains **23.4 fps** (measured uncapped, `V3_TARGET_FPS=0`). The original
cannot be benchmarked in that container at all — the image no longer carries a
loadable MediaPipe — so scaling the live rate by the measured ratio gives roughly
**18 fps** for an equivalent MediaPipe loop. That is an estimate, and it is still
nowhere near 30. At the rate it could sustain, the original scores **27/60 (45%)**.
