...
## 40. Multi-person mode audited against the footage, and two production-stopping bugs found while doing it

**User asked to focus on multi-person from here (single-person is only a baseline) and to
check whether it really tracks several people or is making it up.**

### It tracks real people, but it was reporting ghosts as people

`training/audit_multiperson_tracking.py` (new) draws every tracked person's skeleton, box and
id onto sampled frames so the claim can be checked by eye rather than trusted. Looking at the
frames settles two things at once. The poses are real people -- no repeat of SS27's furniture
-- but a frame labelled `tracked=10` had **four** skeletons and six bare dots at
probability 0.00: tracks being held through a dropout, not people.

Measured over whole clips (detections per frame vs tracks per frame):

| clip | people actually detected | tracks reported | ghosts |
|---|---|---|---|
| 16 | 3.97 | 5.92 | 1.95 |
| 17 | 2.67 | 4.84 | 2.17 |

`camera_manager` was logging `person_count=len(results)`, so **40-45% of the person counts in
the alert log and the UI were stale tracks**. Fixed: `V3MultiPersonFallState.seen_count`
counts only people seen this frame, and that is what gets logged.

### Track churn is a detection problem, not a tracking problem

The tracker creates 19-53 ids on clips with at most 4-7 people (churn 4.75-7.57x), and each
new id restarts that person's 30-frame window. An instrumented count attributed 68-91% of new
ids to "a track existed nearby but was not matched", which pointed at the matcher -- so
ultralytics' **ByteTrack** was wired in as an alternative (`V3_TRACKER=bytetrack`, kept as an
option, not the default).

**ByteTrack did not help**: churn 6.75 -> 9.0 on clip4, and it introduced 2-3 id switches
where the hip matcher had none. That falsifies the diagnosis rather than the tracker: in
crowded frames *some* track is always within the "nearby" radius, so that metric could not
distinguish a failed match from a genuinely new person. The real driver is detection
continuity -- on Test/1 the detector averages 0.83 people per frame, so tracks expire because
nobody is detected for 30 straight frames.

### What actually helped: pose confidence 0.5 -> 0.3

Following the same logic as SS39's imgsz change -- if continuity is the problem, stop
discarding marginal detections. `V3_POSE_CONF` (new, was a hardcoded 0.5, ultralytics' own
default is 0.25):

| pose conf | val falls | val ADL clean | train50 falls | train50 ADL clean | 2-person composite |
|---|---|---|---|---|---|
| 0.50 | 15/15 | 10/16 | 22/25 | 22/25 | 13/15 |
| **0.30** | **15/15** | **10/16** | **24/25** | **22/25** | **13/15** |
| 0.25 | 15/15 | 10/16 | 24/25 | 22/25 | - |

**+2 falls on the held-out train50 set, nothing worse anywhere.** 0.25 measures identically to
0.3, so 0.3 is taken as the smaller change. Deployed.

`docs/model_comparison.md` (new, generated by `training/compare_old_vs_new.py`) is the
old-vs-new table across every surface, regenerable in one command.

### Two bugs found while running the above, both of which would have broken production

**1. Connection exhaustion.** Postgres reached `sorry, too many clients already` and refused
every connection, psql included. Cause: `create_app()` builds a new SQLAlchemy engine -- and
a new pool -- on every call, and it was called per camera task, per notification thread, and
once a minute by the escalation sweep. Nothing disposed them. Fixed with
`app.get_worker_app()`, one cached app per process, plus a bounded pre-pinged pool in
`SQLALCHEMY_ENGINE_OPTIONS`. Measured after: 11-19 connections steady with two cameras and
beat running, versus climbing to 100 and dying.

**2. Escalation never ran.** Beat sent 11 sweeps; the worker received 11 and executed **1**.
Camera detection tasks are infinite loops holding a prefork slot each, so with
`--concurrency=4` and two cameras (fall + alone = 4 tasks) every slot is permanently taken and
beat's tasks queue forever. **The feature that re-alerts when nobody acknowledges a fall was
silently dead**, and would have been dead in production precisely when it mattered. Fixed with
a `maintenance` queue (`task_routes` + the beat entry's `options`) and a dedicated
`celery_maintenance` container. Verified: escalation now completes every 60s while both
cameras run.
